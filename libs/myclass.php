...
class sqlProcess
{
    ...
    function sqlsorgu($requestQuery, $query, $params, $choos = 0, $query2, $params2)
    {
        if (isset($_POST[$requestQuery])) {
            ...
            $name = htmlspecialchars($params[0], ENT_QUOTES, 'UTF-8');
            $mail = htmlspecialchars($params[1], ENT_QUOTES, 'UTF-8');
            $pass = htmlspecialchars($params[2], ENT_QUOTES, 'UTF-8');
            ...
            if ($choos != 0) {
                // Alttaki 3 satır ile verilen params2 ile verilen verinin kaç tane olduğunu sorgu ile aratıyor.
                $sorgu = $db->prepare($query2);
                $guncelle = $sorgu->execute($params2);
                $kayitSayisi = $sorgu->fetchColumn();
                if ($kayitSayisi == 0) {
